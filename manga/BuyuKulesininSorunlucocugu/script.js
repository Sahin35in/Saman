document.addEventListener("DOMContentLoaded", function () {
    ...
    if (localStorage.getItem("buyuViews") === null) {
        localStorage.setItem("buyuViews", "0"); // 🌟 Başlangıç değeri olarak 0 koy
    }

    // 🌟 Kullanıcı sadece belirli URL'ye geldiğinde artır
    if (window.location.pathname.includes("/manga/Buyu-Kulesinin-Sorunlu-cocugu/buyu.html")) {
        let buyuViews = parseInt(localStorage.getItem("buyuViews")) || 0;
        buyuViews += 1; // 🌟 Her girişte artır

        localStorage.setItem("buyuViews", buyuViews);

        if (totalViewsElement) {
            totalViewsElement.innerHTML = `<strong>Görüntülenme:</strong> ${buyuViews}`;
        }
    }
});
document.addEventListener("DOMContentLoaded", function () {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 📌 LocalStorage'dan bölümleri çekme
    let currentPage = window.location.pathname; // 🌟 Geçerli sayfa adını al
